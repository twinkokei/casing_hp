...
switch ($page) {
	case 'list':
		...
		$where_branch = "";
		if($_SESSION['user_type_id']==2 || $_SESSION['user_type_id']==3 || $_SESSION['user_type_id']==4){
	...
	case 'save_payment':
		...
		$branch_id = get_branch_id($transaction_id);
		$data_total = get_data_total($transaction_id);
		$total_discount = get_total_discount($transaction_id);
		//echo $total_discount;
      ...
      $con2 = mysql_connect("localhost", "root", "",true); //online
      mysql_select_db("resto_new",$con1);
      mysql_select_db("resto_admin",$con2);
//                        create_config2("transactions", $data, $con2); //con online



//
      $que =  mysql_query("SELECT flag_code FROM transactions WHERE flag_code = '0'",$con2);
//                        $ser = mysql_fetch_array($que);
//
//                        foreach ($ser as $value) {
//                            mysql_query("UPDATE transactions SET flag_code = '1'",$con1);
//                        }
//
