...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('G1', 'Total')
						...
						->setCellValue('M1', 'Bayar')
						->setCellValue('N1', 'Kembali')
						->setCellValue('O1', 'Kasir');
...
while ($row = mysql_fetch_array($query)) {
	...
	$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('H'.$i, $row['transaction_discount'].'%')//diskon
						...
						->setCellValue('M'.$i, 'Rp. '.number_format($row['transaction_payment']))//bayar
						->setCellValue('N'.$i, 'Rp. '.number_format($row['transaction_change']))//kembali
						->setCellValue('O'.$i, $row['user_name'].'('.$row['user_login'].')');
						$i++;
						$j++;
}
$k = $i++;
